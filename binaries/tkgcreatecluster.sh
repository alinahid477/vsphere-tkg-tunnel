...
{
    local qx=$1
    local firstCharacter=${qx:0:1}
    local lastCharacter=${qx: -1}
    if [[ ! $firstCharacter =~ [\"] ]]
    then
        qx=\"$qx 
    fi
...
function doCreateCluster() {
    ...
    local wizardmode=$1
    local defaultvalue_name=$2 
    local defaultvalue_vsphere_namespace=$3
    local defaultvalue_kubernetes_version=$4
    local defaultvalue_control_plane_count=$5
    local defaultvalue_control_plane_vm_class=$6 
    local defaultvalue_control_plane_storage=$7
    local defaultvalue_worker_node_count=$8 
    local defaultvalue_worker_node_vm_class=$9 
    local defaultvalue_worker_node_storage=${10}
    local defaultvalue_volume_mount_name=${11}
    local defaultvalue_volume_mount_path=${12}
    local defaultvalue_volume_mount_size=${13}
    ...
    then
        ...
        local latestversion=$(kubectl get tkr --sort-by=.metadata.name -o jsonpath='{.items[-1:].metadata.name}' | awk 'NR==1{print $1}')
    ...
    local DEFAULT_STORAGE_CLASS=$(kubectl get storageclass -o jsonpath='{.items[-1:].metadata.name}')
    local CONTROL_PLANE_STORAGE=''
    ...
    local VOLUME_MOUNT_NAME=''
    local VOLUME_MOUNT_PATH=''
    local VOLUME_MOUNT_SIZE=''
    local IS_PRESENT_VOLUME_MOUNT=''
    local VOLUME_CONFIG=''
    if [[ -z $defaultvalue_volume_mount_name || -z $defaultvalue_volume_mount_path || -z $defaultvalue_volume_mount_size ]] 
    then
        while true; do
