...
function doOnboardWorkloadCluster() {
    ...
    local connectedtanzucontext=$(tanzu config server list -o json | jq '.[].context' | xargs)
    ...
    local tanzupackagerepository=$(tanzu package repository list -A | grep tanzupackages)
    ...
    printf "\n"
    local isexist=$(kubectl get storageclass --no-headers --output custom-columns=":metadata.name" | head -n 1)
    if [[ -z $isexist ]]
    then
