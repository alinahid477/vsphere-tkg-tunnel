...
function listsupportedpackages() {
...
function doTanzuInstallPackage() {
    ...
    local packageid=$(tanzu package available list -A | grep -w $packagename | awk '{print $1}' | xargs)
    if [[ -z $packageid ]]
    then
    ...
    local packageversion=$(tanzu package available list $packageid -A --output json | jq '.[].version' | xargs)
    if [[ -z $packageversion ]]
    then
    ...
    then
        ...
        local search=SELECTED_PACKAGE_NAME
        local replace=$packagename
    ...
    local packagedatavalues=$(cat ~/binaries/tanzuwizard/data-file.map | sed -r 's/[[:alnum:]]+=/\n&/g' | awk -F: '$1=="'$packageid'"{print $2}' | xargs)
